...
<script>
		...
		$
				.ajax({
					...
					success : function(data) {
						...
						var pageBarSize = 5;
						var totalPage = Math
								.ceil(parseFloat(total) / viewCount);
						var pageNo = (Math.floor((cPage - 1)
								/ parseFloat(pageBarSize)))
								* pageBarSize + 1;
						...
						if (pageNo == 1) {
							...
							pageNation += '<li class="page-item"><a class="page-link" href="javascript:loadInstructor('
									+ (pageNo - 1)
									+ ','
									+ pageBarSize
									+ ',\'all\')">Previous</a></li>';
						...
						while (!(pageNo > pageEnd || pageNo > totalPage)) {
							console.log("test");
							pageNation += '<li class="page-item"><a class="page-link '
									+ (pageNo == cPage ? "currentPage" : "")
									+ '" href="javascript:loadInstructor('
									+ pageNo
									+ ','
									+ pageBarSize
									+ ',\'all\')">' + pageNo + '</a></li>';
						...
						for (index in data.list) {
							...
							rmHtml += "<td class='text-left'>" + board.TITLE
									+ "</td>";
							rmHtml += "<td class='text-left'>"
									+ (board.CONTENT.replace(/(<([^>]+)>)/ig,
											"")).replace("&nbsp;", "")
											.substring(0, 45) + "</td>";
							...
							rmHtml += "<td><button class='btn btn-primary' onclick='fn_boardModify("
									+ board.BNO + ")'>수정</button></td>";
							rmHtml += "<td><button class='btn btn-primary' onclick='fn_boardView("
									+ board.BNO + ")'>보기</button></td>";
							rmHtml += "</tr>";
						}
	...
	function loadInstructor(cPage, pageBarSize, type) {
		$
				.ajax({
					url : "${rootPath}/rest/board/" + type + "/" + cPage + "/"
							+ pageBarSize,
					dataType : "json",
					success : function(data) {
						...
						var cPage = data.cPage;
						var total = data.total;
						var totalPage = Math.ceil(parseFloat(total)
								/ numPerPage);
						var pageNo = (Math.floor((cPage - 1)
								/ parseFloat(pageBarSize)))
								* pageBarSize + 1;
