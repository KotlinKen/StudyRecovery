...
<jsp:include page="/WEB-INF/views/common/admin_header.jsp"><jsp:param value="${board.TITLE }" name="pageTitle" /></jsp:include>
...
<div class="panel buttonPanel">
	...
	<button type="button" class="btn btn_reg" onclick="javascript:location.href='${rootPath}/admin/boardModify?bno=${board.BNO}'">수정</button> 
	</c:if>
	<button type="button" class="btn btn_reg" onclick="javascript:location.href='${rootPath}/admin/boardList'">목록</button> 
	</div>
</div>
	...
	<form action="${rootPath }/admin/replyWrite" method="post">
		<div class="form-row">
	...
	<div class="form-row">
		...
		<div class="form-group col-md-1">
			<button type="button" class="btn rm_custom_btn" onclick="return fn_commentCheck()">전송</button>
		</div>
	</div>
	</c:if>
	
	
	<form action="${rootPath }/admin/replyDelete" method="post" id="replyHandler">
		<input type="hidden" name="bno" value="${board.BNO}"/>
		<input type="hidden" name="rno" id="rno" />
		<input type="hidden" name="mno" value="${mber.mno }"/>
	</form>
	
	<form action="${rootPath }/admin/replyModify" method="post" id="replyModifyHandler">
		<input type="hidden" name="bno" value="${board.BNO}"/>
...
function loadData(bno, type, cPage, pageBarSize){
	$.ajax({
		url:"${rootPath}/admin/replyList?bno="+bno+"&cPage="+cPage,
		dataType:"json",
	    	...
	    	for(index in data.list){
	    		...
	    		var profile = reply.MPROFILE;
	    			...
	    			rmHtml += "<div class='reply'>"
		    			...
		    			if('${mber.mno}'== '${board.MNO}' && '${memberLoggedIn.mno}' != reply.MNO && '${board.FORK}' == 0 ){ 
...
function fn_fork(mno, bno, rno){
	...
	if(confirm("채택 하시겠습니까?")){
		$.ajax({
	        url : "${rootPath}/admin/boardReplyFork",
	        type : "POST",
