...
<style>
	...
	<jsp:include page="/WEB-INF/views/common/header.jsp"> 
		<jsp:param value="내 정보 보기" name="pageTitle"/>
			...
			<form id="update-form" action="${rootPath }/member/updateUser.do" method="post" enctype="multipart/form-data" onsubmit="return submitConfirm();">
				<c:if test="${memberLoggedIn != null }">
						...
						<c:if test="${!(memberLoggedIn.mprofile eq 'no')}">
							<img src="${rootPath }/resources/upload/member/${memberLoggedIn.mprofile}" alt="${memberLoggedIn.mprofile}" style="width:100px;" id="imgMprofile"/> 
						</c:if>
						<c:if test="${memberLoggedIn.mprofile eq 'no'}">
							<p>프로필 사진이 없습니다.</p>
						</c:if>
						<br />
						<input type="file" name="post_mprofile" id="post-file" />
			...
			<script>
				...
				function submitConfirm(){
					if($("#post-file").val()==""){
						...
						$("input#post-file").attr("name","mprofile");
						console.log("post");
					}
					return true;
				}
